...
interface ContextConfig {
  maxTokens: number;          // Max tokens before compression
  targetTokens: number;       // Target tokens after compression
  keepRecentCount: number;    // Number of recent messages to always keep
  autoCompress: boolean;      // Auto-compress when limit reached
}
...
class SmartContextManager {
   ...
   * Summarize messages
   */
  private async summarize(messages: Message[], existingSummary?: string): Promise<string> {
...
Create a concise summary (max 500 words).`;
    ...
    return `Previous conversation included ${messages.length} messages about: ${
      messages.slice(0, 3).map(m => m.content.slice(0, 50)).join(', ')
    }...`;
  }
  ...
  import(sessionId: string, data: string): boolean {
    try {
      ...
      this.contexts.set(sessionId, {
        ...
        tokenCount: parsed.messages.reduce(
          (sum: number, m: Message) => sum + this.estimateTokens(m.content),
          0
        ) + (parsed.summary ? this.estimateTokens(parsed.summary) : 0),
      });
