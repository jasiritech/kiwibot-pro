...
Please respond in this JSON format:
{
  ...
  async extractText(imageSource: string): Promise<string> {
    const result = await this.analyze(
      imageSource,
      'Extract and return ALL text visible in this image, preserving formatting. Only return the text, nothing else.'
    );
   ...
   * Compare two images
   */
  async compare(image1: string, image2: string): Promise<{
    similar: boolean;
    similarity: number;
    differences: string[];
  }> {
   ...
   */
  async caption(imageSource: string, style: 'short' | 'detailed' | 'funny' = 'short'): Promise<string> {
    const prompts = {
   ...
   */
  private downloadImage(url: string): Promise<{ base64: string; mediaType: string }> {
    ...
    return [
      {
        ...
        execute: async (params: any) => this.analyze(params.imageSource, params.prompt)
      }
    ];
  }
}
