...
interface UsageEvent {
  timestamp: Date;
  type: 'message' | 'command' | 'skill' | 'api_call' | 'error';
  userId: string;
  channel: string;
  details: {
    model?: string;
...
class AnalyticsTracker {
   ...
   */
  getSummary(): {
    total: { messages: number; tokens: number; users: number; cost: number };
    today: { messages: number; tokens: number; users: number; cost: number };
  ...
  private load(): void {
    try {
      ...
      this.dailyStats = new Map(
        data.dailyStats.map(([k, v]: [string, any]) => [
          k,
    ...
    return [
      {
        ...
        execute: async (params: any) => this.getUserStats(params.userId)
      },
      {
        ...
        execute: async (params: any) => this.getSummary()
      }
    ];
  }
}
