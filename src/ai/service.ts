...
class AIService {
   ...
   */
  async *stream(message: Message): AsyncGenerator<string> {
    const session = sessionManager.getOrCreate(
      message.author.id,
      message.channel,
      message.channelId
   ...
   * Execute tool calls
   */
  private async executeToolCalls(toolCalls: AIToolCall[]): Promise<unknown[]> {
  ...
  getModels(): AIModel[] {
    ...
    if (this.anthropic) {
      available.push(
        ...
        MODELS['claude-opus-4-5-20250514']
      );
    }
   ...
   */
  async checkProviders(): Promise<string[]> {
