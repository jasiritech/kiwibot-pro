...
class WorkspacePersonality {
  ...
  private parseSoulMarkdown(content: string): SoulConfig {
    ...
    if (personalityMatch) soul.personality = personalityMatch[1].trim();
   ...
   */
  private parseAgentsMarkdown(content: string): AgentsConfig {
   ...
   */
  private parseExamples(text: string): Array<{ user: string; assistant: string }> {
  ...
  findAgentByTrigger(text: string): (typeof this.agents)['agents'] extends (infer T)[] ? T : never | undefined {
    if (!this.agents?.agents) return undefined;
   ...
   */
  createDefaultSoul(name: string = 'KiwiBot'): void {
    const content = `# ${name}
