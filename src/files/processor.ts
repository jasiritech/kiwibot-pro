...
class FileProcessor {
   ...
   */
  async process(source: string): Promise<ProcessedFile> {
    let filePath: string;
   ...
   */
  async analyze(fileOrContent: string | ProcessedFile): Promise<FileAnalysis> {
    let content: string;
...
Respond in JSON format:
{
   ...
   */
  async extract(file: string | ProcessedFile, query: string): Promise<string> {
    let content: string;
...
Provide a direct, concise answer based only on information in the document.`;
  ...
  private async processCSV(filePath: string): Promise<string> {
    ...
    const table: string[] = [headers.join(' | ')];
    ...
    return [
      {
        ...
        execute: async (params: any) => this.process(params.filePath)
      },
      {
        name: 'file_summarize',
        ...
        execute: async (params: any) => this.analyze(params.content)
      }
    ];
  }
}
