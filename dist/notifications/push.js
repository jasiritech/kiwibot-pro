...
class NotificationService {
    ...
    async notify(userId, title, body, options = {}) {
        ...
        const results = await Promise.all(prefs.channels
            .filter(c => c.enabled)
     ...
     */
    async sendToChannel(channel, title, body, options) {
        try {
            switch (channel.type) {
                case 'webhook':
     ...
     */
    sendSlack(webhookUrl, title, body) {
     ...
     */
    sendDiscord(webhookUrl, title, body) {
        return this.httpPost(webhookUrl, {
            embeds: [
                {
                    title: `🥝 ${title}`,
     ...
     */
    sendTelegram(botToken, chatId, title, body) {
     ...
     */
    removeChannel(userId, channelType) {
        const prefs = this.preferences.get(userId);
        if (!prefs)
            return false;
     ...
     */
    async broadcast(title, body, priority = 'normal') {
        let sent = 0;
     ...
     */
    async scheduleReminder(userId, title, body, delayMs) {
