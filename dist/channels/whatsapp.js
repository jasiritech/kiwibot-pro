...
class WhatsAppChannel {
    ...
    async handleMessage(waMsg) {
        if (waMsg.fromMe)
            return;
        if (waMsg.from === 'status@broadcast')
            return;
        ...
        if (chat.isGroup) {
            ...
            const namesCalled = ['kiwi', 'kiwibot'].some(n => waMsg.body.toLowerCase().includes(n));
            if (!isMentioned && !namesCalled)
                return;
        }
        const message = {
            id: waMsg.id.id,
